...
for design, df in summary_data.items():
    ax1.plot(df['Year'], df['Mean_P_Distance'], 
             ...
             label=design.capitalize(), alpha=0.8)
...
ax1.set_ylabel('Mean P-Distance', fontsize=12, fontweight='bold')
ax1.set_title(f'{lineage} Vaccine Design Performance (P-Distance)', 
              fontsize=14, fontweight='bold')
ax1.legend(loc='best', fontsize=11)
ax1.grid(True, alpha=0.3)

# Plot ML distance (only for medoid)
if 'medoid' in summary_data:
    df = summary_data['medoid']
    valid_ml = df[~df['Mean_ML_Distance'].isna()]
    if len(valid_ml) > 0:
        ax2.plot(valid_ml['Year'], valid_ml['Mean_ML_Distance'],
                marker='s', linewidth=2, markersize=6,
                label='Medoid (ML Distance)', alpha=0.8, color='#2ecc71')
...
ax2.set_ylabel('Mean ML Distance (LG+G4)', fontsize=12, fontweight='bold')
ax2.set_title(f'{lineage} Medoid Performance (ML Distance)',
              fontsize=14, fontweight='bold')
...
# ============================================
# Plot 2: Heatmap of Mean Distances
# ============================================
...
sns.heatmap(heatmap_df.T, annot=True, fmt='.4f', cmap='RdYlGn_r',
            cbar_kws={'label': 'Mean P-Distance'},
            linewidths=0.5, ax=ax)
ax.set_title(f'{lineage} Mean P-Distance by Design and Year',
             fontsize=14, fontweight='bold', pad=20)
ax.set_xlabel('Year', fontsize=12, fontweight='bold')
ax.set_ylabel('Design Type', fontsize=12, fontweight='bold')
...
bars = ax.bar(means.keys(), means.values(), color=colors, alpha=0.8)
ax.set_ylabel('Overall Mean P-Distance', fontsize=11, fontweight='bold')
ax.set_title('Average Performance Across All Years', fontsize=12, fontweight='bold')
...
bars = ax.bar(medians.keys(), medians.values(), color=colors, alpha=0.8)
ax.set_ylabel('Overall Median P-Distance', fontsize=11, fontweight='bold')
ax.set_title('Median Performance Across All Years', fontsize=12, fontweight='bold')
ax.grid(True, alpha=0.3, axis='y')
...
ax.bar(df['Year'], df['N_Strains'], color='#95a5a6', alpha=0.8)
ax.set_xlabel('Year', fontsize=11, fontweight='bold')
ax.set_ylabel('Number of Strains', fontsize=11, fontweight='bold')
ax.set_title('Strains Analyzed Per Year', fontsize=12, fontweight='bold')
ax.grid(True, alpha=0.3, axis='y')

# Plot D: Variability comparison
ax = axes[1, 1]
variability = {design: df['SD_P_Distance'].mean() for design, df in summary_data.items()}
bars = ax.bar(variability.keys(), variability.values(), color=colors, alpha=0.8)
ax.set_ylabel('Average Standard Deviation', fontsize=11, fontweight='bold')
ax.set_title('Distance Variability Across Years', fontsize=12, fontweight='bold')
ax.grid(True, alpha=0.3, axis='y')
